...
enum USER_TYPE {
  ...
  kAssistUser     = 1,       // 助教
  kTeacherUser    = 2,       // 讲师
  kShopMasterUser = 3,       // 学校|门店管理员
  kShopOwnerUser  = 4,       // 学校|门店拥有者
  kMaintainUser   = 5,       // 运营维护者
  kAdministerUser = 6,       // 系统管理员
};

// define client type...
enum CLIENT_TYPE {
  kClientPHP       = 1,       // 网站终端...
  kClientStudent   = 2,       // 学生终端...
  kClientTeacher   = 3,       // 老师终端...
  kClientUdpServer = 4,       // UDP服务器...
  kClientScreen    = 5,       // 屏幕终端...
};

// 注意：定义与服务器交互命令编号 => 从1开始编号...
// 注意：PHP 代码中也可以使用 __LINE__
const long CMD_LINE_START   = __LINE__ + 1;
enum {
  ...
  kCmd_UDP_Logout           = __LINE__ - CMD_LINE_START,
  kCmd_Camera_PullStart     = __LINE__ - CMD_LINE_START,
  kCmd_Camera_PullStop      = __LINE__ - CMD_LINE_START,
  kCmd_Camera_OnLineList    = __LINE__ - CMD_LINE_START,
  kCmd_Camera_LiveStart     = __LINE__ - CMD_LINE_START,
  kCmd_Camera_LiveStop      = __LINE__ - CMD_LINE_START,
